...
export function TransmissionChainCard({
  ...
  return (
    <section className="w-full rounded-2xl border border-slate-200/90 bg-gradient-to-b from-white to-slate-50/50 p-5 shadow-sm dark:border-slate-800 dark:from-zinc-950 dark:to-zinc-900/40 sm:p-6 text-slate-800 dark:text-slate-100 my-3">
      {/* Header */}
      <header className="mb-5 flex flex-col sm:flex-row sm:items-start justify-between gap-3.5 border-b border-slate-100 pb-4 dark:border-slate-800">
        <div>
          <div className="flex flex-wrap items-center gap-2 mb-1.5">
            <span className="text-xs font-bold uppercase tracking-wider text-cyan-600 dark:text-cyan-400 bg-cyan-50 dark:bg-cyan-950/50 px-2.5 py-1 rounded-full border border-cyan-200 dark:border-cyan-800">
              DeepEar 邏輯傳導鏈
            </span>
      ...
      <div className="space-y-3.5 mb-5">
        ...
        <div className="grid gap-3.5">
          {data.chain.map((step, idx) => (
            <div
              key={idx}
              className="relative rounded-xl border border-slate-200/80 bg-white dark:bg-zinc-900/90 dark:border-slate-800 p-4 shadow-xs transition-all hover:border-cyan-500/40"
            >
              <div className="flex items-start justify-between gap-3 mb-2">
                <div className="flex items-center gap-2.5">
                  <span className="flex size-6 items-center justify-center rounded-full bg-slate-900 text-xs font-bold text-white dark:bg-slate-100 dark:text-slate-900 shrink-0">
                    {step.step}
                  </span>
              ...
              {step.affectedSectors && step.affectedSectors.length > 0 && (
                <div className="mt-2.5 ml-8 flex flex-wrap gap-1.5">
                  {step.affectedSectors.map((sec, sIdx) => (
                    <span
                      key={sIdx}
                      className="text-xs bg-slate-50 dark:bg-zinc-800 text-slate-600 dark:text-slate-400 px-2.5 py-0.5 rounded border border-slate-200/60 dark:border-slate-700/60 font-mono"
                    >
                      {sec}
                    </span>
                  ))}
      ...
      <div className="rounded-xl border border-red-500/20 bg-red-50/50 dark:bg-red-950/20 p-4 sm:p-5 mb-4">
        <h4 className="text-sm font-bold text-red-900 dark:text-red-300 mb-2 flex items-center gap-1.5">
          <span>🚨 核心論點證偽判定點 (Falsification Criteria)</span>
        </h4>
        <p className="text-xs sm:text-sm text-red-700/90 dark:text-red-300/90 mb-3">
          若市場或企業出現以下任一狀況，代表原投資邏輯已被「證偽 (Falsified)」，應嚴格啟動風險停損：
        </p>
        <ul className="space-y-2 text-xs sm:text-sm text-red-900 dark:text-red-200 font-medium">
      ...
      {data.actionableInsights && (
        <div className="rounded-xl border border-slate-200/70 bg-slate-50/70 dark:bg-zinc-900/60 dark:border-slate-800/80 p-4 flex items-start gap-3 text-xs sm:text-sm">
          <div className="text-base text-cyan-600 dark:text-cyan-400 shrink-0 font-bold">
            💡
          </div>
          <div>
